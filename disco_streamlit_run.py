# MultureLabModelBeta1 [w/ 3D animation] (modified by @softology to work on Visions of Chaos and further modified by @multure to run on MULTUREAILAB)
        ...
        for frame_num in range(args.start_frame, args.max_frames):
          ...
          """
            ...
            for i in range(args.n_batches):
                ...
                save_filename = f"{DefaultPaths.output_path}/{sanitize_filename(args2.prompt)} [MultureLabModelBeta1] {args2.seed}.png"
                print(save_filename)
                file_list = []
                if path_exists(save_filename):
                    for file in sorted(os.listdir(f"{DefaultPaths.output_path}/")):
                        if file.startswith(
                            f"{sanitize_filename(args2.prompt)} [MultureLabModelBeta1] {args2.seed}"
                        ):
                            print(file)
                            file_list.append(file)
                    print(file_list)
                    ...
                    if last_name[-15:-10] == "batch":
                        ...
                        count_string = f"{count_value:05d}"
                        save_filename = f"{DefaultPaths.output_path}/{sanitize_filename(args2.prompt)} [MultureLabModelBeta1] {args2.seed}_batch {count_string}.png"
                    else:
                        save_filename = f"{DefaultPaths.output_path}/{sanitize_filename(args2.prompt)} [MultureLabModelBeta1] {args2.seed}_batch 00001.png"
                shutil.copyfile(
                    args2.image_file,
                    save_filename,
                )
